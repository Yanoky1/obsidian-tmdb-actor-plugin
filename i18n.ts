...
export interface Translations {
	...
	settings: {
		...
		actorFolder: string;
		actorFolderDesc: string;
		
		actorTemplate: string;
		actorTemplateDesc: string;
		
		actorFileNameFormat: string;
		actorFileNameFormatDesc: string;
 ...
 */
export function initializeLanguage(lang?: string | SupportedLanguage) {
	// Если язык не указан или не поддерживается, используем auto-detect
	if (!lang || (lang !== 'ru' && lang !== 'en')) {
		const browserLang = navigator.language.split('-')[0];
		currentLanguage = (browserLang === 'ru' || browserLang === 'en') 
